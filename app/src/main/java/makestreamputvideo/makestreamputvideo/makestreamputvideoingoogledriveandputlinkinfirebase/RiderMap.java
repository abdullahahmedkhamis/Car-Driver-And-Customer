...
public class RiderMap extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, OnMapReadyCallback,
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        for (DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                            MediaSession.Token token = postSnapshot.getValue( MediaSession.Token.class); //Get Token object from Database with key
...
//                          Make Raw payload - Convert LatLng to JSON
                        ...
                        //    Notification data = new Notification(riderToken, json_lat_lng); //Send it to Driver then we deserialize it
                         //   Sender content = new Sender(token.getToken(), data); //Send the same data to the token
                            Sender content = new Sender(token);
...
//                        Add driver to map
    ...
    public void onBackPressed() {
        @SuppressLint("WrongViewCast") DrawerLayout drawer = (DrawerLayout) findViewById(R.id.drawer_layout11);
